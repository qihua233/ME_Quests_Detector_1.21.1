...
### 修复
- **右键离线检测器无任何提示**:此前 `DetectorBlock.useItemOn` 在 `POWERED=false` 时直接返回 SUCCESS,不发送动作栏文案。现在补齐使用已有的 `ae2-ftbquests-detector.detector.uncharged`("设备离线" / "Device Offline")提示,与无队伍、队伍不存在、网络冲突等其它失败状态保持一致(没装 Jade 的玩家也能感知)。
